...
class Handler:
    def __init__(self, filepath: str):
        ...
        self.totalname = join(dirname, self.uchar_filename)
        self.mc_list = list()

    def execute(self, *, _batch_size=5):
...
def execute(f):
    try:
        Handler(f).execute()
        send_msg(f"Успешно {f}", delete_after=10)
    except Exception as e:
        import traceback
        tb_str = ''.join(traceback.format_exception(type(e), e, e.__traceback__))
        send_msg(f"{basename(__file__)}: Ошибка обработки файла {f}\n{tb_str}")
